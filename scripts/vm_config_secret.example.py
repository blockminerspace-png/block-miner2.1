# Copia para vm_config_secret.py e preenche (vm_config_secret.py está no .gitignore).
#
#   copy vm_config_secret.example.py vm_config_secret.py   (Windows)
#   cp vm_config_secret.example.py vm_config_secret.py     (Linux/macOS)

# Default test VM (production deploy uses deploy.secrets / deploy.py — not this file).
IP = "89.167.114.67"
LOGIN = "root"
ROOT_PASSWORD = "sua_senha_aqui"
# Optional: benchmark login (gitignored file — never commit real values)
# BLOCKMINER_TEST_IDENTIFIER = "email-de-teste-sem-2fa"
# BLOCKMINER_TEST_PASSWORD = "senha"
# Repo on the test server: https://github.com/blockminerspace-png/block-miner-v3.git
