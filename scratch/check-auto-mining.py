...
def main():
    ...
    queries = [
        ("auto_mining_gpu_logs count", "SELECT COUNT(*) FROM user_inventory WHERE miner_name IN ('Pulse GPU v1', 'GPU 1 GHS');"),
        ("auto_mining_gpus in database", "SELECT * FROM auto_mining_gpus;"),
        ("auto_mining_gpu_logs in database", "SELECT COUNT(*) FROM auto_mining_gpu_logs;")
    ]
