...
function mergeReplyUnique(replies: SupportReply[], incoming: SupportReply): SupportReply[] {
  if (!incoming?.id) return replies;
  if (replies.some((r: SupportReply) => r.id === incoming.id)) return replies;
...
export default function Support() {
  ...
  const [list, setList] = useState<SupportListRow[]>([]);
  ...
  const fetchList = useCallback(
    async (p = 1, append = false) => {
      try {
        setListLoading(true);
        const res = await getSupportTickets({ page: p, limit });
        if (res.data?.ok) {
          const rows = (res.data.messages || []) as SupportListRow[];
          setList((prev: SupportListRow[]) => (append ? [...prev, ...rows] : rows));
  ...
  const loadThread = useCallback(
    async (id: number | string) => {
      try {
        ...
        if (res.data?.ok) {
          setThread(res.data.message as SupportThread);
        } else {
  ...
  const onRealtimeReply = useCallback((reply: unknown) => {
    const incoming = reply as SupportReply;
    setThread((prev: SupportThread | null) => {
      if (!prev || Number(prev.id) !== Number(incoming.supportMessageId)) return prev;
      return {
        ...prev,
        replies: mergeReplyUnique(prev.replies || [], incoming)
      };
    });
  }, []);

  useSupportTicketSocket(selectedId == null ? null : Number(selectedId), onRealtimeReply);

  const selectTicket = (id: number | string) => {
    setSelectedId(id);
    setReplyText('');
    setPendingFiles([]);
    loadThread(id);
  ...
  const uploadImages = async (files: File[]) => {
    const urls: SupportAttachment[] = [];
    for (const file of files) {
      const fd = new FormData();
      fd.append('image', file);
      const res = await postSupportUploadImage(fd);
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const res = await postSupportReply(selectedId, {
        message: replyText.trim() || t('support_tickets.reply_image_only'),
        attachments
      });
      if (res.data?.ok && res.data.reply) {
        const reply = res.data.reply as SupportReply;
        setThread((prev: SupportThread | null) =>
          prev
            ? { ...prev, replies: mergeReplyUnique(prev.replies || [], reply) }
            : prev
        );
        setReplyText('');
        setPendingFiles([]);
  ...
  const handleCreateTicket = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await postSupportTicket({
        name: user.name,
        email: user.email,
  ...
  const addFiles = (
    ...
    const next = Array.from(fileList ?? []).filter((f: File) => f.type.startsWith('image/'));
    setter((prev: File[]) => [...prev, ...next].slice(0, max));
  };
          ...
          {threadLoading ? (
            ...
            <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
              ...
              <div className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain p-4 space-y-4">
                ...
                {(thread.replies || []).map((r: SupportReply) => (
                  <div
                    key={r.id}
