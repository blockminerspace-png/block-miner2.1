...
function EarningsChartsPanelInner({ totals, history }: Props) {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <div className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-950/80 p-5 min-h-[320px]">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">
          {t('powerStats.earnings.origin_title')}
        </h3>
        <p className="text-[11px] text-slate-600 mb-4">{t('powerStats.earnings.origin_note')}</p>
        {pieData.length === 0 ? (
      ...
      <div className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-950/80 p-5 min-h-[320px]">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">
          {t('powerStats.earnings.evolution_title')}
        </h3>
        <p className="text-[11px] text-slate-600 mb-4">{t('powerStats.earnings.evolution_note')}</p>
        {lineData.length === 0 ? (
          <p className="text-sm text-slate-600 py-16 text-center">{t('powerStats.charts.no_data')}</p>
        ) : (
          <ResponsiveContainer width="100%" height={260}>
