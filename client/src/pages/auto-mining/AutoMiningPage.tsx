...
export default function AutoMining() {
  ...
  const [turboImpression, setTurboImpression] = useState<TurboImpression | null>(null);
  ...
  const modeRaw = session?.mode || selectedMode;
  const mode: MiningMode = modeRaw === "TURBO" ? "TURBO" : "NORMAL";
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      ...
      setBannerTracked(false);
      try {
        const res = await api.get<{ success?: boolean; impression?: TurboImpression | null }>(
          "/auto-mining-gpu/v2/banner",
        );
        if (res.data.success) {
          setTurboImpression((res.data.impression as TurboImpression | null | undefined) ?? null);
          setTurboBannerError(false);
        }
      } catch (err: unknown) {
        turboFetchedForRef.current = null;
  ...
  const registerBannerClick = async (impressionId: string) => {
    try {
  ...
  const handleClaimTurbo = async (e: MouseEvent<HTMLButtonElement>) => {
    if (!validateTrustedEvent(e)) return;
    if (!turboImpression?.id || !bannerTracked) return;
    setActionBusy(true);
    try {
      const res = await api.post<AutoMiningV2Payload>("/auto-mining-gpu/v2/claim/turbo", {
        impressionId: turboImpression.id,
      });
      if (res.data.success) {
              ...
              {!isRunning ? (
                <div className="space-y-6">
                  ...
                  <AutoMiningModeSelector
                    value={selectedMode}
                    onChange={(m: string) => setSelectedMode(m === "TURBO" ? "TURBO" : "NORMAL")}
