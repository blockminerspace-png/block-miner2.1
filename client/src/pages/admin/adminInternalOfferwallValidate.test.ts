...
import {
  ...
  type InternalOfferwallFormInput,
} from './adminInternalOfferwallValidate';

function baseForm(overrides: Partial<InternalOfferwallFormInput> = {}): InternalOfferwallFormInput {
  return {
    kind: 'PTC_IFRAME',
    title: 'Test offer',
    description: '',
...
describe('validateAdminInternalOfferwallForm', () => {
  ...
  it('rejects empty title', () => {
    ...
    if (!r.ok) expect(r.i18nKey).toBe('admin_internal_offerwall.validation_title_required');
  });

  it('rejects invalid iframe URL for PTC', () => {
    const r = validateAdminInternalOfferwallForm(baseForm({ iframeUrl: 'https://' }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.i18nKey).toBe('admin_internal_offerwall.validation_iframe_invalid');
  });

  it('rejects empty iframe for PTC', () => {
    const r = validateAdminInternalOfferwallForm(baseForm({ iframeUrl: '' }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.i18nKey).toBe('admin_internal_offerwall.validation_iframe_required');
  });

  it('rejects invalid ISO2 country codes', () => {
    const r = validateAdminInternalOfferwallForm(baseForm({ targetCountryCodes: 'BRA, US' }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.i18nKey).toBe('admin_internal_offerwall.validation_country_iso2');
  });
