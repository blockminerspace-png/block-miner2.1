...
type TransparencyType = 'expense' | 'income';

type TransparencyPeriod = 'monthly' | 'annual' | 'daily' | 'one_time';

type TransparencyDirection = 'in' | 'out';
...
type TransparencyEntry = {
  ...
  direction?: TransparencyDirection | string;
...
type WalletActivitySummary = {
  totalInPol?: number | string | null;
  totalInUsd?: number | string | null;
  totalOutPol?: number | string | null;
  totalOutUsd?: number | string | null;
...
function readJsonMessage(data: unknown): string | undefined {
...
const EMPTY_FORM: TransparencyFormState = {
  type:           'expense',
    ...
    <div className="space-y-6" data-testid="admin-transparency">
      ...
      <div className="rounded-2xl border border-sky-500/20 bg-sky-500/5 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_160px_160px]">
          ...
          <input className={inputCls} type="number" min="0" value={trackedWalletForm.sortOrder} onChange={(e) => setTrackedWalletForm((f) => ({ ...f, sortOrder: Number.parseInt(e.target.value, 10) || 0 }))} />
        </div>
              ...
              {entries.map(entry => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <span className="text-[11px] text-gray-500 ml-1">/{getPeriodLabel(entry.period ?? 'monthly')}</span>
