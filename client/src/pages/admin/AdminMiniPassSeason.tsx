...
type RewardCatalogKind = "SHOP_MINER" | "EVENT_MINER";
...
function normalizeRewardCatalogItem(row: AdminMinerCatalogRow): RewardCatalogItem {
  const rawId = row?.id;
...
function isGameSlugMissionType(missionType: string): boolean {
...
export default function AdminMiniPassSeason() {
  ...
  const [seasonId, setSeasonId] = useState<number | null>(isNew ? null : parseInt(id ?? "", 10));
  ...
  const [missionDraft, setMissionDraft] = useState<MissionDraftState>({
    cadence: "EVENT",
  ...
  const rewardCatalogKind: RewardCatalogKind | null =
    rewardDraft.rewardKind === "SHOP_MINER" || rewardDraft.rewardKind === "EVENT_MINER" ? rewardDraft.rewardKind : null;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const s: AdminMiniPassSeasonDetail = res.data.season;
  ...
  }, [rewardCatalogKind, rewardCatalogQuery, t]);
  ...
  const saveSeason = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      if (isNew) {
        const res = await api.post<AdminMiniPassSeasonWriteResponse>("/admin/mini-pass/seasons", payload);
        if (res.data.ok && res.data.season?.id) {
          toast.success(t("adminMiniPass.season.created"));
          navigate(`/admin/mini-pass/${res.data.season.id}`);
        }
      } else {
        await api.put<AdminMiniPassSeasonWriteResponse>(`/admin/mini-pass/seasons/${seasonId}`, payload);
  ...
  const deleteReward = async (rid: number) => {
  ...
  const deleteMission = async (mid: number) => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-2" id="mp-banner">
                    <ImageUploader value={form.bannerImageUrl} onChange={(url: string) => setForm({ ...form, bannerImageUrl: url })} />
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setForm({
                        ...form,
                        maxLevel: Number.isFinite(v) ? Math.max(1, Math.min(500, v)) : form.maxLevel,
                      });
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setForm({
                        ...form,
                        xpPerLevel: Number.isFinite(v) ? Math.max(1, Math.min(1_000_000, v)) : form.xpPerLevel,
                      });
                    }}
                  />
                </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setRewardDraft({
                            ...rewardDraft,
                            level: Number.isFinite(v) ? Math.max(1, Math.min(500, v)) : rewardDraft.level,
                          });
                        }}
                      />
                    </div>
