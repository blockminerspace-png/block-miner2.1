...
type SidebarNavGetResponse = {
  ok: boolean;
  entries?: SidebarNavEntry[];
  itemMeta?: Record<string, SidebarItemMeta>;
};

type SidebarNavPutResponse = {
  ok: boolean;
  entries?: SidebarNavEntry[];
  code?: string;
};
...
function groupBySection(
  rows: SidebarNavEntry[],
  itemMeta: Record<string, SidebarItemMeta>,
): { section: string; rows: SidebarNavEntry[] }[] {
  const map = new Map<string, SidebarNavEntry[]>();
...
export default function AdminUserSidebar() {
  ...
  const [itemMeta, setItemMeta] = useState<Record<string, SidebarItemMeta>>({});
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get<SidebarNavGetResponse>('/admin/sidebar-nav');
      if (!res.data?.ok) {
  ...
  const updateEntry = (itemId: string, patch: Partial<SidebarNavEntry>) => {
    setEntries((prev) => prev.map((e) => (e.itemId === itemId ? { ...e, ...patch } : e)));
  };

  const swapSortOrder = (itemIdA: string, itemIdB: string) => {
  ...
  const moveInList = (itemId: string, direction: 'up' | 'down') => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.put<SidebarNavPutResponse>('/admin/sidebar-nav', { entries });
      if (!res.data?.ok) {
