...
function readAxiosResponseStatus(err: unknown): number | undefined {
...
function toastStreamingFailure(t: TFunction, errLike: unknown) {
  const status = readAxiosResponseStatus(errLike);
  const msg = readAxiosResponseMessage(errLike) || '';
...
type StreamingRowWorkerFields = {
  workerAlive?: boolean;
  lastWorkerStatus?: string;
  desiredRunning?: boolean;
};

function rowDisplayStatus(r: StreamingRowWorkerFields) {
  if (r.workerAlive) return 'LIVE';
  return String(r.lastWorkerStatus || 'OFFLINE');
}

function rowIsStarting(r: StreamingRowWorkerFields) {
  return Boolean(
    r.desiredRunning &&
...
type CreateFormState = ReturnType<typeof emptyCreateForm>;

interface StreamingDestinationRow extends StreamingRowWorkerFields {
  id: number;
  label: string;
  captureUrl: string;
  rtmpUrl: string;
...
interface StreamingDestinationsListResponse {
  ok?: boolean;
  destinations?: StreamingDestinationRow[];
}

interface StreamingMutationResponse {
  ok?: boolean;
  message?: string;
}
...
export default function AdminStreaming() {
  ...
  const [rows, setRows] = useState<StreamingDestinationRow[]>([]);
  ...
  const load = useCallback(
    async (opts: { silent?: boolean } = {}) => {
      ...
      try {
        const res = await api.get<StreamingDestinationsListResponse>('/admin/streaming/destinations');
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      const res = await api.post<StreamingMutationResponse>('/admin/streaming/destinations', body);
      if (res.data?.ok) {
  ...
  const start = async (id: number) => {
    setBusyId(id);
    try {
      const res = await api.post<StreamingMutationResponse>(`/admin/streaming/destinations/${id}/start`);
      if (res.data?.ok) {
      ...
      toast.error(readAxiosResponseMessage(e) || t('admin_streaming.load_error'));
    } finally {
      setBusyId(null);
    }
  };
  ...
  const stop = async (id: number) => {
    setBusyId(id);
    try {
      const res = await api.post<StreamingMutationResponse>(`/admin/streaming/destinations/${id}/stop`);
      if (res.data?.ok) {
  ...
  const remove = async (id: number) => {
    ...
    setBusyId(id);
    try {
      const res = await api.delete<StreamingMutationResponse>(`/admin/streaming/destinations/${id}`);
      if (res.data?.ok) {
