...
type InternalOfferwallSaveBody = {
  kind: string;
  title: string;
  description: string | null;
...
function defaultForm(): InternalOfferwallFormState {
  return {
    kind: KIND_PTC,
    title: '',
    description: '',
...
function rowToForm(row: InternalOfferwallOfferRow): InternalOfferwallFormState {
...
function buildApiBody(form: InternalOfferwallFormState): InternalOfferwallSaveBody {
...
function rewardSummary(row: InternalOfferwallOfferRow): string {
...
function currentRewardSummary(form: InternalOfferwallFormState): string {
...
function formatInternalOfferwallApiError(t: TFunction, data: InternalOfferwallApiErr | undefined): string {
...
function readAxiosApiErr(err: unknown): InternalOfferwallApiErr | undefined {
...
type OffersListResponse = { ok: boolean; offers?: InternalOfferwallOfferRow[] };
type AttemptsListResponse = { ok: boolean; attempts?: InternalOfferwallAttemptRow[] };
type SaveMutationResponse = { ok: boolean } & Partial<InternalOfferwallApiErr>;

export default function AdminInternalOfferwall() {
  const { t } = useTranslation();
  const [tab, setTab] = useState<'offers' | 'review'>('offers');
  ...
  const loadOffers = useCallback(async () => {
    ...
    try {
      const res = await api.get<OffersListResponse>('/admin/internal-offerwall/offers');
  ...
  const loadAttempts = useCallback(async () => {
    ...
    try {
      const res = await api.get<AttemptsListResponse>('/admin/internal-offerwall/attempts?status=PENDING_REVIEW&limit=100');
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const res = editingId
        ? await api.patch<SaveMutationResponse>(`/admin/internal-offerwall/offers/${editingId}`, body)
        : await api.post<SaveMutationResponse>('/admin/internal-offerwall/offers', body);
      if (res.data?.ok) {
      ...
      toast.error(formatInternalOfferwallApiError(t, readAxiosApiErr(e)) || readAxiosResponseMessage(e) || t('admin_internal_offerwall.load_error'));
    } finally {
      setSaving(false);
    }
  };

  const toggleActive = async (row: InternalOfferwallOfferRow) => {
    setBusyId(row.id);
    try {
      const res = await api.patch<SaveMutationResponse>(`/admin/internal-offerwall/offers/${row.id}`, { isActive: !row.isActive });
      if (res.data?.ok) {
        toast.success(t('admin_internal_offerwall.save_ok'));
        await loadOffers();
      } else {
  ...
  const approve = async (id: number) => {
    setBusyId(id);
    try {
      const res = await api.post<SaveMutationResponse>(`/admin/internal-offerwall/attempts/${id}/approve`);
      if (res.data?.ok) {
        toast.success(t('admin_internal_offerwall.save_ok'));
        await loadAttempts();
      } else {
  ...
  const reject = async (id: number) => {
    setBusyId(id);
    try {
      const res = await api.post<SaveMutationResponse>(`/admin/internal-offerwall/attempts/${id}/reject`, {
