...
import { getShopMiners, postShopPurchase } from './shop.api';
...
export default function Shop() {
    ...
    const [selectedMiner, setSelectedMiner] = useState<ShopCatalogMiner | null>(null);
    ...
    useEffect(() => {
        const fetchMiners = async () => {
            try {
                ...
                if (res.data.ok && res.data.miners) {
    ...
    const openConfirmModal = (miner: ShopCatalogMiner) => {
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            const res = await postShopPurchase({
            ...
            const d = isAxiosError(err) ? err.response?.data : undefined;
            const errMsg =
                d && typeof d === 'object' && 'messageKey' in d && typeof (d as { messageKey?: unknown }).messageKey === 'string'
                    ? t((d as { messageKey: string }).messageKey)
    ...
    return (
        <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                {miners.map((miner: ShopCatalogMiner) => (
                            ...
                            <div className="bg-gray-900/50 border border-gray-800 rounded-3xl p-6 space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-right">
                                        <span className="text-xl font-black text-white italic">{(Number(selectedMiner.price) * quantity).toFixed(2)} <span className="text-xs font-bold text-gray-500 not-italic uppercase">POL</span></span>
                                        {quantity > 1 && <p className="text-[9px] text-gray-600 font-bold">{String(selectedMiner.price)} POL × {quantity}</p>}
                                    </div>
                                </div>
                            </div>
