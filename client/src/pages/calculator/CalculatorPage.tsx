...
} from '../../shared/utils/calculatorEngine';
...
export default function CalculatorPage() {
    ...
    useEffect(() => {
        api.get('/shop/miners?pageSize=48')
            .then(res => { if (res.data.ok) setMiners((res.data.miners || []) as ShopMiner[]); })
            .catch(() => {})
                            ...
                            <div className="space-y-2">
                                ...
                                {!myHashManual && (stats?.miner?.estimatedHashRate ?? 0) > 0 ? (
                                    <p className="text-[10px] text-green-400 font-bold flex items-center gap-1">
                                        ...
                                        {t('calculator.synced_inventory', { value: formatHashrate(stats?.miner?.estimatedHashRate ?? 0) })}
                                    </p>
                                ) : hashFromMiners > 0 ? (
