...
function RackCard({ rackIndex, rackName, rackBaseSlot, machines }: RackCardProps) {
    const slots = useMemo(() => (
...
export default function PublicRoom() {
    ...
    const machines: RoomMiner[] = targetUser?.miners || [];
    const racks = targetUser?.racks || {};
    const roomList: RoomInfoRow[] = targetUser?.rooms || [];

    const totalHashRate = useMemo(() =>
        machines.reduce((s: number, m) => s + Number(m.hashRate || m.hash_rate || 0), 0) + (targetUser?.gamePower || 0),
    [machines, targetUser]);
