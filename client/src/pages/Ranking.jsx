...
function MiniRacks({ username, navigate }) {
  ...
  if (!data) {
    return <div className="py-8 text-center text-sm text-slate-600">Não foi possível carregar a sala.</div>;
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-black text-slate-400 uppercase tracking-widest">
            {machines.length} máquina(s) ativa(s) · {formatHashrate(machines.reduce((s, m) => s + (m.hashRate || 0), 0))}
          </span>
        </div>
        <button
