...
describe('sidebarNavMap', () => {
  it('maps API category with group and children', () => {
    ...
    const first = menu[0]?.items?.[0];
    expect(first?.type).toBe('group');
    if (!first || first.type !== 'group') throw new Error('expected group item');
    const group = first as SidebarMenuGroup;
    expect(group.children).toHaveLength(1);
    expect(group.children[0]?.path).toBe('/faucet');
  });
  ...
  it('normalizeMiniPassOutOfRewardsGroup pulls mini_pass out of Rewards children', () => {
    ...
    if (!group?.children) throw new Error('expected rewards group children');
