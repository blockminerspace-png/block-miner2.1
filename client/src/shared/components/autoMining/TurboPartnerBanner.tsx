...
export type TurboPartnerImpression = { id: string; title?: string };

type TurboPartnerBannerProps = {
  impression: TurboPartnerImpression | null;
  loading?: boolean;
  error?: boolean;
  disabled?: boolean;
...
export default function TurboPartnerBanner({
  ...
  async function handleOpen(e: MouseEvent<HTMLButtonElement>) {
    const imp = impression;
    if (!imp) return;
    if (!validateTrustedEvent(e)) return;
    e.preventDefault();
    await onRegisterClick(imp.id);
