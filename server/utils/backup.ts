...
async function createDatabaseBackup({ run, backupDir, filenamePrefix, logger }) {
  ...
  try {
    ...
    if (logger && logger.warn) {
      logger.warn("VACUUM INTO failed; falling back to file copy", { error: errMsg(error) });
    }
  }
  ...
  for (const entry of entries) {
    ...
    if (stat.mtimeMs < cutoffMs) {
      try {
        ...
        if (logger && logger.warn) {
          logger.warn("Failed to delete old backup", { file: fullPath, error: errMsg(error) });
        }
      }
    }
  }
...
async function createFullSiteBackup({ backupDir, filenamePrefix, logger }) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on("error", (error: unknown) => {
      reject(new Error(`Tar command failed: ${errMsg(error)}`));
    });
