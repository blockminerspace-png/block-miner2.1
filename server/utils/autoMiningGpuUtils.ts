...
import {
  ...
  getAllReleasedGPUs,
  getGPUReport,
} from "../models/autoMiningGpuModel.js";
...
async function testGetAvailable(userId = 1) {
  ...
  try {
    ...
    gpus.forEach(gpu => {
      console.log(`  - ID: ${gpu.id}, Hash Rate: ${gpu.gpuHashRate} GHS, Disponível: ${gpu.isAvailable}`);
    });
    return gpus;
  } catch (err) {
    console.error("✗ Erro:", err);
...
async function testGetHistory(userId = 1, limit = 10) {
  ...
  try {
    ...
    history.forEach(log => {
      console.log(`  - ID: ${log.id}, Ação: ${log.action}, Fonte: ${log.source}, Data: ${new Date(log.claimedAt).toLocaleString()}`);
    });
    return history;
  } catch (err) {
    console.error("✗ Erro:", err);
...
async function simulateCronBehavior(userIds = [1, 2, 3, 4, 5]) {
  ...
  for (const userId of userIds) {
    try {
      ...
      console.error(`✗ Erro para usuário ${userId}:`, err instanceof Error ? err.message : String(err));
    }
  }
