...
async function createWithdrawal(userId, amount, address, feeAmount = 0) {
  return prisma.$transaction(async (tx) => {
    ...
    const user = await tx.user.findUnique({
      where: { id: userId },
      select: { id: true, username: true, email: true, registrationIp: true, ip: true, polBalance: true },
    });
    ...
    await createTelegramOutboxEventTx(tx, TELEGRAM_EVENT_TYPES.WITHDRAWAL_REQUESTED_PRIVATE_ALERT, {
      ...transaction,
      user,
    });
...
async function updateTransactionStatus(transactionId: number, status: string, txHash: string | null = null) {
  const now = new Date();
  await prisma.$transaction(async (tx) => {
    ...
    if (transaction.type === 'withdrawal') {
      ...
      if (status === 'completed' && txHash) {
        await createTelegramOutboxEventTx(tx, TELEGRAM_EVENT_TYPES.WITHDRAWAL_COMPLETED_PUBLIC_PROOF, updated, { txHash });
      }
