...
function timingSafeEqualToken(a: unknown, b: unknown) {
...
const expectedToken = getSecretValue("PHD_INTERNAL_TOKEN");
...
app.post("/internal/hd/addresses", async (req, res) => {
  try {
    ...
    logger.error("internal/hd/addresses failed", { error: errMsg(err) });
...
app.listen(port, "0.0.0.0", () => {
  ...
  if (process.env.POLYGON_HD_AUTO_SWEEP === "1") {
    ...
    sweepHdDepositAddressesOnce().catch((err: unknown) =>
      logger.warn("HD sweep startup run error", { error: errMsg(err) })
    );
    cron.schedule(sweepCron, () => {
      sweepHdDepositAddressesOnce().catch((err: unknown) =>
        logger.warn("HD sweep cron error", { error: errMsg(err) })
      );
    });
    logger.info(`POLYGON_HD_AUTO_SWEEP=1 — sweep cron ${sweepCron}`);
  } else {
    logger.warn("POLYGON_HD_AUTO_SWEEP is disabled; HD wallets will not auto-send to treasury.");
  }
});
