...
rankingRouter.get("/room/:username", requireAuth, async (req, res) => {
  try {
    ...
    const mappedMiners = targetUser.miners.map((m: (typeof targetUser.miners)[number]) => ({
      id: m.id,
      hashRate: m.hashRate,
    ...
    const gamePower =
      targetUser.gamePowers.reduce((sum: number, p: { hashRate: number | null }) => sum + (p.hashRate || 0), 0) +
      targetUser.ytPowers.reduce((sum: number, p: { hashRate: number | null }) => sum + (p.hashRate || 0), 0) +
      (targetUser.gpuAccess || []).reduce((sum: number, p: { gpuHashRate: number | null }) => sum + (p.gpuHashRate || 0), 0);

    const racks: Record<number, string | null> = {};
    targetUser.rackConfigs.forEach((config: (typeof targetUser.rackConfigs)[number]) => {
      racks[config.rackIndex] = config.customName;
    });

    const unlockedRooms = new Set(targetUser.userRooms.map((r: (typeof targetUser.userRooms)[number]) => r.roomNumber));
