...
function prismaClientErrorFields(error: unknown): { code?: string; meta?: unknown } {
...
async function resolveReferrerFromRefInput(refCodeInput: unknown) {
...
function buildAccessCookie(accessToken: string) {
...
function buildRefreshCookie(refreshToken: string, expiresAt: number) {
 ...
 */
function timingSafeAdminSecretEqual(supplied: unknown, expectedFromEnv: string | undefined) {
...
function signPasswordResetToken(userId: number) {
...
function verifyPasswordResetToken(token: unknown): (JwtPayload & { typ?: string }) | null {
  try {
...
authRouter.post(
  ...
  try {
    ...
    if (Boolean((user as { isTwoFactorEnabled?: boolean }).isTwoFactorEnabled)) {
