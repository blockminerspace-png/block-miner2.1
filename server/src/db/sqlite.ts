...
export function run(
  sql: string,
  params: (string | number | null | Buffer)[] = [],
  ...
  return new Promise((resolve, reject) => {
    db.run(sql, params, function onRun(this: sqlite3.RunResult, error: Error | null) {
      ...
      resolve({ lastID: Number(this.lastID), changes: Number(this.changes) });
    });
  });
}

export function get(sql: string, params: (string | number | null | Buffer)[] = []): Promise<Record<string, unknown> | null> {
  return new Promise((resolve, reject) => {
...
export function all(sql: string, params: (string | number | null | Buffer)[] = []): Promise<Record<string, unknown>[]> {
  return new Promise((resolve, reject) => {
...
export async function initializeDatabase() {
  ...
  const faucetMinerSlug = sqliteConfig.faucet?.rewardMinerSlug || "faucet-1ghs";
  ...
  let faucetMinerId: number | null = null;
  if (!faucetMinerRow) {
    ...
    faucetMinerId = Number(faucetMinerRow.id);
  ...
  if (!existingFaucetReward) {
    ...
    if (faucetMiner?.id != null) {
      const now = Date.now();
      const cooldownMs = Number(sqliteConfig.faucet?.cooldownMs || 3600000);
      await run(
        "INSERT INTO faucet_rewards (miner_id, cooldown_ms, is_active, created_at, updated_at) VALUES (?, ?, 1, ?, ?)",
        [Number(faucetMiner.id), cooldownMs, now, now]
      );
    }
  }
