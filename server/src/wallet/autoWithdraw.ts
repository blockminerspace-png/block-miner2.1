...
export async function processApprovedWithdrawal(withdrawalId: number): Promise<{ success: boolean; txHash: string }> {
    ...
    try {
        ...
        logger.error(`Failed to process withdrawal ${withdrawalId}`, { error: errMsg(error) });

        const msg = error instanceof Error ? error.message : errMsg(error);
        const status = msg === "INSUFFICIENT_FUNDS" ? "approved" : "failed";
        await prisma.transaction.update({
            where: { id: withdrawalId },
            data: { status }
        });
        throw new Error(msg === "INSUFFICIENT_FUNDS" ? "Insufficient hot wallet funds." : "Transaction failed on-chain.");
    }
}
