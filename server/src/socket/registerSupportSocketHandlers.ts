 ...
 */
function allowJoin(key: string, max = 60, windowMs = 60000) {
  const now = Date.now();
...
export function registerSupportSocketHandlers({
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("support:subscribe", async (payload: unknown, callback) => {
      try {
        ...
        const jwtPayload = authToken ? verifyAccessToken(authToken) : null;
    ...
    socket.on("support:subscribeAdmin", async (payload: unknown, callback) => {
      try {
        const p = payload && typeof payload === "object" && !Array.isArray(payload) ? (payload as Record<string, unknown>) : {};
        const token = typeof p.token === "string" ? p.token : undefined;
        const supportMessageId = p.supportMessageId;
