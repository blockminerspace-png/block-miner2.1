...
function normalizeRackIds(raw: unknown): number[] {
  if (!Array.isArray(raw)) return [];
  return [...new Set(raw.map((value) => Number(value)).filter((value) => Number.isInteger(value) && value > 0))];
}

type RackMoveBackRow = { id: number; roomId: number; userId: number };
type MinerWithMinerRel = Prisma.UserMinerGetPayload<{ include: { miner: true } }>;
...
export async function buyRoom(req: Request, res: Response) {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    const userId = user.id;
    ...
    // Verificar saldo
    const balanceUser = await prisma.user.findUnique({ where: { id: userId }, select: { polBalance: true } });
    if (!balanceUser) {
...
export async function installMiner(req: Request, res: Response) {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    const userId = user.id;
...
export async function uninstallMiner(req: Request, res: Response) {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    const userId = user.id;
...
export async function uninstallMinerBatch(req: Request, res: Response) {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    const userId = user.id;
...
export async function getSlotsSummary(req: Request, res: Response) {
  try {
    const user = requireSessionUser(req, res);
    if (!user) return;
    const userId = user.id;
