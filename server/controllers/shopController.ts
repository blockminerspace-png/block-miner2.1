...
function isShopPurchaseReplayPayload(
  v: unknown,
): v is { newBalance: number; quantity: number; minerName: string; totalPrice: number } {
  if (typeof v !== "object" || v === null) return false;
  const o = v as Record<string, unknown>;
  return (
    typeof o.newBalance === "number" &&
    typeof o.quantity === "number" &&
    typeof o.minerName === "string" &&
    typeof o.totalPrice === "number"
  );
}

export async function listMiners(req: Request, res: Response) {
  try {
    ...
    logger.error("listMiners error", { message: readErrorMessage(error) }, req);
    res.status(500).json({ ok: false, message: "Unable to load miners." });
  }
}

export async function purchaseMiner(req: Request, res: Response) {
  ...
  try {
    ...
    if (phase.type === "replay") {
      ...
      if (!isShopPurchaseReplayPayload(p)) {
        logger.error("purchaseMiner replay payload invalid shape", { minerId, quantity }, req);
        return res.status(500).json({ ok: false, message: "Purchase replay data invalid." });
      }
      return res.json({
        ok: true,
        idempotent: true,
    ...
    try {
      txResult = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        await advisoryXactTryLockOrThrow(tx, `shop_purchase:${user.id}`);
        await lockUserRowForUpdate(tx, user.id);
          ...
          if (currentMiner.maxPerUser != null) {
            const ownedCount = await tx.userOwnedMachine.count({ where: { userId: user.id, minerId: miner.id } });
            if (ownedCount + quantity > Number(currentMiner.maxPerUser)) throw new Error("Miner purchase limit reached.");
          }

          const userRow = await tx.user.findUnique({ where: { id: user.id } });
          const balanceBefore = Number(userRow?.polBalance || 0);
      ...
      await abortIdempotencyLease(lease);
      const errMsg = readErrorMessage(error);
      if (["Miner unavailable.", "Miner out of stock.", "Miner purchase limit reached."].includes(errMsg)) {
        logger.warn("purchaseMiner rejected", { minerId, quantity, reason: errMsg }, req);
      ...
      if (errCode === "P2034") {
        logSecurityWarn("SHOP_PURCHASE_TX_CONFLICT", { minerId, quantity, prismaCode: errCode }, req);
        return res.status(409).json(buildSecurityErrorJson(SecurityErrorCodes.RACE_CONDITION_DETECTED));
      }
      if (errCode === "DISTRIBUTED_LOCK_BUSY") {
        logSecurityWarn("SHOP_PURCHASE_LOCK_BUSY", { minerId, quantity }, req);
        return res.status(409).json(buildSecurityErrorJson(SecurityErrorCodes.RACE_CONDITION_DETECTED));
      }
      logger.error("purchaseMiner transaction error", { minerId, quantity, message: errMsg, code: errCode }, req);
    ...
    applyUserBalanceDelta(user.id, -txResult.totalPrice);
    ...
    logger.error("purchaseMiner fatal error", { message: readErrorMessage(error), code: readErrorCode(error) }, req);
