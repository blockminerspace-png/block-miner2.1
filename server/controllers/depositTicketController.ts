...
type DepositTicketStatusFilter = (typeof DEPOSIT_TICKET_STATUSES)[number];

function parseDepositTicketStatusQuery(raw: unknown): Prisma.DepositTicketWhereInput | undefined {
  if (raw == null || raw === "") return undefined;
  const s = typeof raw === "string" ? raw.trim() : String(raw).trim();
  if (s === "" || s === "all") return undefined;
  if ((DEPOSIT_TICKET_STATUSES as readonly string[]).includes(s)) {
    return { status: s as DepositTicketStatusFilter };
  }
  return undefined;
}
 ...
 */
function respondDepositTicketDbError(
  res: Response,
  error: unknown,
  op: "approve" | "reject" | "get",
): Response {
...
export async function createTicket(req: Request, res: Response) {
  try {
    ...
    logger.error("createTicket error", { error: readErrorMessage(error) });
...
export async function listMyTickets(req: Request, res: Response) {
  try {
    ...
    logger.error("listMyTickets error", { error: readErrorMessage(error) });
...
export async function adminListTickets(req: Request, res: Response) {
  try {
    ...
    const statusWhere = parseDepositTicketStatusQuery(req.query.status);
    const where: Prisma.DepositTicketWhereInput = statusWhere ?? {};
    ...
    logger.error("adminListTickets error", { error: readErrorMessage(error) });
    return res.status(500).json({ ok: false, message: "Erro ao listar tickets." });
  }
}
...
export async function adminGetTicket(req: Request, res: Response) {
  try {
    ...
    logger.error("adminGetTicket error", {
...
export async function adminApproveTicket(req: Request, res: Response) {
  try {
    ...
    const priorCredited =
      resolveTicketOnly && existingDepositByHash ? Number(existingDepositByHash.amount) : null;

    let priorFailureKey: string | null = null;
    ...
    logger.error("adminApproveTicket error", {
      message: readErrorMessage(error),
      code: readErrorCode(error),
    });
...
export async function adminRejectTicket(req: Request, res: Response) {
  try {
    ...
    logger.error("adminRejectTicket error", {
      message: readErrorMessage(error),
      code: readErrorCode(error),
    });
...
type DepositTicketOnChainInput = {
  txHash: string | null;
  walletAddress: string;
};

async function analyzeOnChain(ticket: DepositTicketOnChainInput): Promise<OnChainScanResult> {
  const result: OnChainScanResult = {
    found: false,
    txHash: ticket.txHash || null,
    from: null,
    to: null,
  ...
  try {
    ...
    if (ticket.txHash) {
      ...
      const [txData, receiptData] = (await Promise.all([txRes.json(), receiptRes.json()])) as [
        { result?: { hash?: string; from?: string; to?: string; value?: string; blockNumber?: string } },
      ...
      if (tx && tx.hash) {
        result.found = true;
        result.from = tx.from?.toLowerCase() ?? null;
        result.to = tx.to?.toLowerCase() ?? null;
        result.value = tx.value ?? null;
    ...
    if (ticket.walletAddress && DEPOSIT_WALLET) {
      ...
      if (listData?.result && Array.isArray(listData.result)) {
        ...
        const toUs = listData.result
          .filter(
            (row: EtherscanTxListEntry) =>
              row.to?.toLowerCase() === DEPOSIT_WALLET &&
              row.isError === "0" &&
              Number(row.timeStamp) >= cutoff,
          )
          .slice(0, 20)
          .map((row: EtherscanTxListEntry) => ({
            hash: String(row.hash ?? ""),
            value: (parseInt(String(row.value ?? "0"), 10) / 1e18).toFixed(8),
    ...
    result.error = readErrorMessage(err);
    logger.warn("analyzeOnChain error", { error: readErrorMessage(err) });
  }
