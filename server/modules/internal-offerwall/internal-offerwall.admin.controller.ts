...
export async function createOffer(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!parsed.ok) {
      const payload: { ok: false; message: string; code?: string; details?: unknown } = {
        ok: false,
...
export async function listAttempts(req: Request, res: Response): Promise<void> {
  try {
    ...
    const rows = await adminListAttempts({ status: status || undefined, offerId: offerIdFilter, limit });
...
export async function approveAttempt(req: Request<AttemptParams>, res: Response): Promise<void> {
  try {
    ...
    if (!out.ok) {
      res.status((out as { status?: number }).status ?? 500).json({ ok: false, message: out.message });
      return;
    }
    res.json({ ok: true });
  } catch (e: unknown) {
