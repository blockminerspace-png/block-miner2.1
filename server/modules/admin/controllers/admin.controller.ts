...
import type { PendingWithdrawalForCollision } from "../../../services/adminAccountCollisionService.js";
...
function parseBoolInput(v: unknown): boolean {
...
function serializeMinerForAdmin(m: Miner | null | undefined) {
  if (!m) return m;
  return {
    id: m.id,
    name: m.name,
...
function parseMinerWriteBody(b: unknown): ParsedMinerWrite {
...
/** JSON shape expected by `client/src/pages/admin/AdminMetrics.tsx` */
export async function getServerMetrics(_req: Request, res: Response) {
  try {
    ...
    logger.error("getServerMetrics failed", { error: errMsg(error) });
...
export async function getStats(_req: Request, res: Response) {
  try {
    ...
    logger.error("Admin stats error", { error: errMsg(error) });
    res.status(500).json({ ok: false, message: "Unable to load admin stats." });
  }
}

export async function listRecentUsers(req: Request, res: Response) {
  try {
...
export async function setUserBan(req: Request, res: Response) {
  try {
...
export async function listMiners(_req: Request, res: Response) {
  try {
...
export async function createMiner(req: Request, res: Response) {
  try {
    ...
    logger.error("Admin createMiner", { error: errMsg(error), code: prismaErrCode(error) });
    const msg =
      prismaErrCode(error) === "P2002"
        ? "Slug must be unique."
        : errMsg(error) || "Creation failed";
    res.status(500).json({ ok: false, message: msg });
  }
}

export async function updateMiner(req: Request, res: Response) {
  try {
    ...
    logger.error("Admin updateMiner", { error: errMsg(error), code: prismaErrCode(error) });
    const code = prismaErrCode(error);
    const msg =
      code === "P2002"
        ? "Slug must be unique."
        : code === "P2025"
          ? "Miner not found."
          : errMsg(error) || "Update failed";
    res.status(500).json({ ok: false, message: msg });
  }
}

export async function listPendingWithdrawals(_req: Request, res: Response) {
  try {
    const withdrawals = (await walletModel.getPendingWithdrawals()) as PendingWithdrawalForCollision[];
...
function isValidPolygonTxHash(h: unknown): boolean {
