...
const logger = loggerLib.child("adminUserInsights");
...
function parseLedgerPagination(req: Request) {
...
export async function getUserWalletLedger(req: Request<UserIdParams>, res: Response): Promise<void> {
  try {
...
export async function getUserActivitySummary(req: Request<UserIdParams>, res: Response): Promise<void> {
  try {
    const userId = parseUserIdParam(req);
    if (!userId) {
      res.status(400).json({ ok: false, message: "Invalid user id" });
