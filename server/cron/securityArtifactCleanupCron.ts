...
export type SecurityArtifactCleanupHandle = { timer: ReturnType<typeof setInterval>; name: string };

export function startSecurityArtifactCleanupCron(): SecurityArtifactCleanupHandle[] {
  ...
  const timer = setInterval(() => {
    void (async () => {
      try {
        ...
        logger.error("Security artifact cleanup failed", { message: errMsg(e) });
      }
    })();
  }, intervalMs);
