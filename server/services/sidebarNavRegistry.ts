...
export const SIDEBAR_SECTIONS: readonly SidebarSection[] = ["main", "earn", "social"];
 ...
 */
export function validateSidebarEntriesPayload(
  entries: unknown
): { ok: true; entries: SidebarPersistedEntry[] } | { ok: false; code: string } {
  ...
  for (const e of entries) {
    ...
    const id = (e as { itemId: string }).itemId;
  ...
  const hasGroup = entries.some((x) => (x as { itemId: string }).itemId === "rewards_group");
  const childOfGroup = entries.filter((x) => (x as { parentItemId: string | null }).parentItemId === "rewards_group");
  if (childOfGroup.length > 0 && !hasGroup) return { ok: false, code: "rewards_group_required" };
  const typed: SidebarPersistedEntry[] = entries.map((e) => {
    const r = e as Record<string, unknown>;
    return {
      itemId: String(r.itemId),
      visible: Boolean(r.visible),
      sortOrder: Number(r.sortOrder),
      section: r.section as SidebarSection,
