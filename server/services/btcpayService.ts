...
class BtcpayRequestError extends Error {
  readonly status: number;
  readonly details: unknown;
  constructor(message: string, status: number, details: unknown) {
    super(message);
    this.name = "BtcpayRequestError";
    this.status = status;
...
export async function createBtcpayInvoice({ amountUsd, metadata }) {
  ...
  const checkout: { speedPolicy: string; paymentMethods?: string[] } = { speedPolicy: "MediumSpeed" };
  ...
  if (!res.ok) {
    ...
    throw new BtcpayRequestError("BTCPAY_INVOICE_CREATE_FAILED", res.status, json);
  }
  return json;
}
...
export async function fetchBtcpayInvoice(invoiceId) {
  ...
  if (!res.ok) {
    throw new BtcpayRequestError("BTCPAY_INVOICE_FETCH_FAILED", res.status, json);
  }
  return json;
}
