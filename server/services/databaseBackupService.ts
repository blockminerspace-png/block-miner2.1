...
async function countPublicTables(prisma: PrismaClient) {
 ...
 */
export async function collectPublicTableExactRowCounts(prisma: PrismaClient, tableNames: string[]) {
...
export async function createPostgresSqlBackup(opts: {
  ...
  return {
    ...
    totalDataRows: exact?.totalDataRows,
    publicTablesWithRows: exact?.publicTablesWithRows,
    publicTablesEmpty: exact?.publicTablesEmpty,
    criticalRowCounts: exact?.criticalRowCounts,
