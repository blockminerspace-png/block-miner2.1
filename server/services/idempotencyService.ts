...
export async function beginIdempotencyLease(p) {
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const data: Record<string, unknown> = isPlainRecord(row?.data) ? row.data : {};
      ...
      if (data.phase === "lease" && Number(data.leaseUntilMs ?? 0) > now) {
        if (data.requestHash != null && data.requestHash !== "" && data.requestHash !== p.requestHash) return { type: "mismatch" };
        return { type: "busy" };
      }
...
/**
 * @param {Lease & { _mem?: unknown; _pg?: unknown }} lease
...
export async function commitIdempotencyResult(lease, body) {
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      const data: Record<string, unknown> = isPlainRecord(row?.data) ? row.data : {};
      if (String(data.leaseToken ?? "") !== String(leaseToken)) return;
      const donePayload = {
        v: 1,
        phase: "done",
        requestHash: body.requestHash,
