...
async function getOrCreateReadEarnGameId(tx: Prisma.TransactionClient) {
...
export async function redeemReadEarnCampaign({
  ...
  logger?: RedeemReadEarnLogger | null;
}) {
  ...
  try {
    ...
    logger?.error?.("readEarn redeem failed", { err: errMsg(e), userId, campaignId });
    return { ok: false, code: REDEEM_GENERIC };
  }
}
