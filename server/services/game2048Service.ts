...
export async function getGame2048Status(userId, now = new Date()) {
  const { cooldownEndsAt, active, rewardDuration } = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
...
export async function applyGame2048Move(userId, sessionId, direction, now = new Date()) {
  ...
  return prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    const initial = await tx.game2048Session.findFirst({
      where: { id: sid, userId }
    });
    if (!initial) {
 ...
 * @param {Date} [now]
 */
export async function claimGame2048Reward(userId, sessionId, meta: Record<string, unknown> = {}, now = new Date()) {
  const sid = Math.floor(Number(sessionId));
  if (!sid) {
    return { ok: false, code: "INVALID_SESSION", status: 400 };
  }
