...
function parseDecimalNumber(value, { max = 1_000_000_000, required = true } = {}) {
...
function parseIntRange(value, { min = 0, max = 1_000_000, required = false, fallback = null } = {}) {
...
export function validateMinerImageUrl(value) {
  const s = String(value || "").trim();
  if (!s) return null;
...
export function parseAdminMinerQuery(query = {}) {
...
export function parseMinerWriteBody(body = {}, { partial = false } = {}) {
  const data = {};
...
export function minerSelect(extra = {}) {
  return {
    id: true,
    name: true,
    slug: true,
...
export async function listAdminMiners(prisma, query = {}) {
  ...
  return { ok: true, ...parsed, total, miners: rows.map(minerDto) };
}

export async function getAdminMiner(prisma, id) {
  ...
  return row ? { ok: true, miner: minerDto(row) } : null;
}

async function audit(prisma, action, minerId, details = {}) {
...
export async function createAdminMiner(prisma, body) {
  const data = parseMinerWriteBody(body);
  ...
  return { ok: true, miner: minerDto(row) };
}

export async function updateAdminMiner(prisma, id, body) {
  const minerId = parseMinerId(id);
  const data = parseMinerWriteBody(body, { partial: true });
...
export async function duplicateAdminMiner(prisma, id) {
  const minerId = parseMinerId(id);
...
export async function archiveAdminMiner(prisma, id) {
  return setMinerState(prisma, id, { isArchived: true, isActive: false, showInShop: false }, "ADMIN_MINER_ARCHIVE");
}

export async function toggleAdminMinerStore(prisma, id, showInShop) {
  return setMinerState(prisma, id, { showInShop: Boolean(showInShop) }, "ADMIN_MINER_TOGGLE_STORE");
}

export async function toggleAdminMinerActive(prisma, id, isActive) {
  return setMinerState(prisma, id, { isActive: Boolean(isActive) }, "ADMIN_MINER_TOGGLE_ACTIVE");
}

async function setMinerState(prisma, id, data, action) {
  const minerId = parseMinerId(id);
  const row = await prisma.miner.update({ where: { id: minerId }, data, select: minerSelect() });
  await audit(prisma, action, minerId, data);
  return { ok: true, miner: minerDto(row) };
}

function parseMinerId(value) {
  const s = String(value || "").trim();
